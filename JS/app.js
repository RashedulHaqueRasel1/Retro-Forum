...
const loadAllData = () => {
  fetch("https://openapi.programming-hero.com/api/retro-forum/posts")
    .then((res) => res.json())
    .then((data) => showCardData(data.posts));

};


//  All Latest post Data Function
const loadAllLatestPost = () => {
  fetch("https://openapi.programming-hero.com/api/retro-forum/latest-posts")
    .then((res) => res.json())
    .then((latestPosts) => latestPostData(latestPosts));
...
// All post Data
const showCardData = (posts) => {

  posts.forEach(post => {
    // console.log(post);
    let isActiveBtn = '';
    if (post.isActive) {
      isActiveBtn = `<span class="flex w-3 absolute ml-20  badge bg-green-500 rounded-full"></span>`;
    }
    else {
      isActiveBtn = `<span class="flex w-3 absolute ml-20 badge bg-red-500 rounded-full"></span>`;
    }
    ...
    <div class="lg:ml-10">
    ${isActiveBtn}
      <img class="w-full h-full lg:w-[100px] lg:h-[100px] rounded-lg" src="${post.image}" alt="">
    </div>
    <div class="ml-6 mt-6 lg:mt-0 lg:ml-10">
...
const latestPostData = (latestPosts) => {
  ...
  latestPosts.forEach(latestPost => {
    ...
    <div class="card w-96 container mx-auto  bg-slate-300 shadow-xl  ">
