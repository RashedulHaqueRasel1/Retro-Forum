...
const showCardData = (searchID) => {
  ...
  searchID.forEach(post => {
    ...
    let isActiveBtn = '';
    if (post.isActive) {
      isActiveBtn = `<span class="flex w-8 h-8 lg:w-5 lg:h-5 me-3 absolute   ml-[355px] lg:ml-20 bg-green-500 rounded-full"></span>`;
    }
    else {
      isActiveBtn = `<span class="flex w-8 h-8 lg:w-5 lg:h-5 me-3 absolute ml-[355px] lg:ml-20 bg-red-500 rounded-full"></span>`;
    }
